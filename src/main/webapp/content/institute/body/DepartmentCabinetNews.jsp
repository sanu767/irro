...
<div id="savetp" >
<s:form cssStyle="width:1px;height: 1px;" action="DepartmentsCabinetNewsSave" method="POST">
	<s:hidden name="departmentBean.news" value="%{departmentBean.news}"/>
<s:submit cssClass="btn btn-primary" value="Submit" onclick="save()"/> 
</s:form>
</div>
<s:if test="%{#session.permissionMap['Кабинет «ОБЖ и физического воспитания»']}">
<a id="canceltp" href="DepartmentsCabinetNews" ><button class="btn btn-link" type="button" value="Cancel">Cancel</button></a>
</s:if>
...
<script>
	...
	function save()	{
		document.getElementById("DepartmentsCabinetNewsSave_departmentBean_news").value=CKEDITOR.instances.editor.getData();
	};
</script>
</body>
</html>