...
<div id="savetp" >
<s:form cssStyle="width:1px;height: 1px;" action="DepartmentsITNewsSave" method="POST">
	<s:hidden name="departmentBean.news" value="%{departmentBean.news}"/>
<s:submit cssClass="btn btn-primary" value="Submit" onclick="save()"/> 
</s:form>
</div>
<s:if test="%{#session.permissionMap['Кафедра информационных технологий']}">
<a id="canceltp" href="DepartmentsITNews" ><button class="btn btn-link" type="button" value="Cancel">Cancel</button></a>
</s:if>
...
<script>
	...
	function save()	{
		document.getElementById("DepartmentsITNewsSave_departmentBean_news").value=CKEDITOR.instances.editor.getData();
	};
</script>
</body>
</html>