...
<body>
<s:if test="%{#session.permissionMap['Кабинет «ОБЖ и физического воспитания»']}">
  <a id="edittp" ><button class="btn btn-primary" type="button" value="Edit" onclick="edit()">Edit</button></a>
</s:if>
  
<div id="savetp" >
<s:form cssStyle="width:1px;height: 1px;" action="DepartmentsCabinetConstitutionSave" method="POST">
...
<s:if test="%{#session.permissionMap['Кабинет «ОБЖ и физического воспитания»']}">
<a id="canceltp" href="DepartmentsCabinetConstitution" ><button class="btn btn-link" type="button" value="Cancel">Cancel</button></a>
</s:if>
...
<script>
	...
	function save()	{
		document.getElementById("DepartmentsCabinetConstitutionSave_departmentBean_constitution").value=CKEDITOR.instances.editor.getData();
	};
</script>
</body>
</html>